...
*ui_options* `str-to-str-map`::
    ...
    interface implementation. The Terminal UI supports the following options:
        ...
        *terminal_cursor_native*:::
            if set, make the terminal cursor visible, on top of Kakoune's cursor.
            Can be set to *inherit* (don't change the shape),
            *default* (reset to the default shape),
            *blinking-block*, *steady-block*,
