...
the system one at `${runtime}/autoload` will not load anymore. You can load
it by adding this command to `${userconf}/kakrc` or a `.kak` file in the
`${userconf}/autoload` directory:

-------------------------------------------------------------------------------------------
evaluate-commands %sh{
    find "${kak_runtime}"/autoload/ -type f -name '*.kak' |
